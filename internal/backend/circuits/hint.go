...
var mulBy7 = hint.NewFixedHint(func(curveID ecc.ID, inputs []*big.Int, result []*big.Int) error {
	result[0].Mul(inputs[0], big.NewInt(7)).Mod(result[0], curveID.Info().Fr.Modulus())
	return nil
}, 1, 1)

var make3 = hint.NewFixedHint(func(curveID ecc.ID, inputs []*big.Int, result []*big.Int) error {
